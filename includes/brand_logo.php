...
if (!function_exists('render_brand_logo')) {
    ...
    function render_brand_logo(array $options = []): void
    {
        ...
        echo '<img src="' . htmlspecialchars($logoPath, ENT_QUOTES, 'UTF-8') . '" alt="' . htmlspecialchars($alt, ENT_QUOTES, 'UTF-8') . '" class="brand-logo__image" loading="lazy" onerror="this.onerror=null; this.classList.add(\'is-hidden\'); if (this.nextElementSibling) { this.nextElementSibling.classList.remove(\'is-hidden\'); }" />';

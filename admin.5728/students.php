...
$students = $db->fetchAll("
    SELECT u.*, COUNT(DISTINCT e.course_id) as enrolled_courses
    FROM users u
...
<head>
    ...
    <title>Students - StudySmart</title>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="row g-3 mb-4">
            ...
            <div class="col-md-4">
                <div class="stat-card">
                    <h3><?php echo count(array_filter($students, fn($s) => $s['is_active'])); ?></h3>
                    <p><i class="fas fa-check-circle me-2"></i>Active Students</p>
                </div>
            </div>
            <div class="col-md-4">
                <div class="stat-card">
                    <h3><?php echo array_sum(array_column($students, 'enrolled_courses')); ?></h3>
                    <p><i class="fas fa-book me-2"></i>Total Enrollments</p>
                </div>
            </div>
        </div>

        <div class="card">
            <div class="card-header"><h5 class="mb-0"><i class="fas fa-user-graduate me-2"></i>All Students</h5></div>
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table">
                        <thead><tr><th>Name</th><th>Email</th><th>Phone</th><th>Courses</th><th>Status</th><th>Joined</th><th>Actions</th></tr></thead>
                        <tbody>
                            <?php foreach ($students as $s): ?>
                            <tr>
                                <td>
                                    <div class="d-flex align-items-center">
                                        <div class="user-avatar me-3" style="width:35px;height:35px;font-size:0.875rem;"><?php echo strtoupper(substr($s['first_name'], 0, 1)); ?></div>
                                        <strong><?php echo htmlspecialchars($s['first_name'] . ' ' . $s['last_name']); ?></strong>
                                    </div>
                                </td>
                                <td><?php echo htmlspecialchars($s['email']); ?></td>
                                <td><?php echo htmlspecialchars($s['phone'] ?? '-'); ?></td>
                                <td><span class="badge bg-primary"><?php echo $s['enrolled_courses']; ?></span></td>
                                ...
                                <td><a href="users.php?action=edit&id=<?php echo $s['id']; ?>" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a></td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>

...
$view_video = null;
if (isset($_GET['id'])) {
    $view_video = $db->fetch("SELECT r.*, c.title as course_title FROM resources r JOIN courses c ON r.course_id = c.id JOIN enrollments e ON c.id = e.course_id WHERE r.id = ? AND e.student_id = ? AND r.resource_type = 'video' AND r.is_active = 1 AND e.is_active = 1", [$_GET['id'], $current_user['id']]);
}
...
<head>
    ...
    <title><?php echo htmlspecialchars($view_video['title']); ?> - StudySmart</title>
...
<body>
    ...
    <div class="main-content">
        <div class="top-nav">
            <h1><i class="fas fa-video"></i>Watch Video</h1>
            <div class="user-info">
